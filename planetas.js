...
async function loadCharacters(url){
    ...
    try{
        ...
        responseJson.results.forEach((characters) =>{
            ...
            `url('https://starwars-visualguide.com/assets/img/planets/${characters.url.replace(/\D/g, '')}.jpg')`
            ...
            cards.onclick = () =>{
                ...
                height.innerText = `Populacao: ${convertHeight(characters.population)}`

                const mass = document.createElement('span')
                mass.className = 'content-details'
                mass.innerText = `diametro: ${convertMass(characters.diameter)}`

                const eyeColors = document.createElement('span')
                eyeColors.className = 'content-details'
                eyeColors.innerText = `clima: ${convertEyeColors(characters.climate)}`

                const birth = document.createElement('span')
                birth.className = 'content-details'
                birth.innerText = `Rotacao: ${convertBirth(characters.rotation_period)}`
...
const convertEyeColors = (eyeColors) => {
    if(eyeColors === 'temperate') return 'Arido'  
    if(eyeColors === 'tropical') return 'tropical'  
    if(eyeColors === 'frozen') return 'congelado'  //as temperaturas mais comuns
    if(eyeColors === 'murky') return 'obscuro'  
    if(eyeColors === 'hot') return 'quente'  
    return eyeColors
}
const convertHeight = (height) =>{
    if(height === 'undefined') return 'desconhecida' //populaçao
    return (height / 100).toFixed(2)
}
const convertMass = (mass) =>{
    if(mass === 'undefined') return 'desconhecido' //diametro
    if(mass === 'unknown') return 'desconhecido' 
    return `${mass}`
}
const convertBirth = (birth) =>{
    if(birth === 'undefined') return 'desconhecido' //rotaçao do planeta
    return `${birth} horas`
}